...
- Final root-cause review found `renderMonth()` was overwriting every busy cell's specific note with the generic unavailable label after `calendarDayStatus()` had already returned the hold countdown. That overwrite was removed. Production verification also found the Render bot had not yet deployed the booking-hold endpoint (`404` despite the commit being on `main`), so the bot deployment must be live before end-to-end verification can pass.
